...
We thank Reviewer reaf for the careful reading and for recognizing the paper's structural reframing, comprehensive evaluation, and honest assessment of the field. We address each point below. New experiments (significance testing and the k>2 study) will be added to the revision, with full tables in the appendix.
...
- The ≤25% figure averaged each score only over queries with a *nonzero achievable metric* —
  ...
  weight has any room to help. Following the more common practice, we now report over the **full
  eval set** (all qrels queries, 0-fill for the rest), the same convention as our new significance
  analysis. Under this evaluation the recovered headroom is substantially higher: query-adaptive
  methods reach up to **61% on NQ (RM3+Qwen3)** and exceed 25% on several MSMARCO/NQ
  configurations, and our new k=3 methods reach **40–46%** on MSMARCO/NQ.
- Headroom nonetheless persists on the harder configurations and no method reaches the oracle, so
  the paper's core point stands: the bottleneck is the per-query predictability of the optimal
  weight, not the fusion mechanism (§5.2).

We will report Tables 1–2 under this full-qrels convention in the revision.

---

## W2 — "Restricted to a single sparse–dense pair (k=2); generalizability to k>2 is unclear."

We agree, and we ran additional experiments on the multi-retriever simplex. We fuse
**BM25 + RM3 + Qwen3** (Qwen3 is the strongest dense retriever on every dataset; BM25 and RM3
trade places, giving a genuine 3-way mix), and re-run two strongest k=2 methods against a
k=3 RRF baseline. Significance is per-query Wilcoxon signed-rank vs that baseline.
...
Three takeaways: (1) the method **extends to k>2 and still beats RRF** (both large benchmarks,
p<0.001); (2) the **headroom grows with k**. The oracle ceiling rises and the oracle−RRF gap is
larger at k=3 than any k=2 pair, so the k=2 study is a *lower bound* on achievable headroom; and
(3) **achieving it is harder**. The k=3 simplex has ~5151 weight settings vs 101 at k=2, so the
per-query optima scatter and no longer collapse into a compact, learnable interval. Full details go in the appendix.
...
RRF using the **Wilcoxon signed-rank test**, with a **Bonferroni correction** over the family of
128 tests (8 query-adaptive methods × 16 dataset–combinations; corrected α = 0.05/128 ≈ 3.9×10⁻⁴).

Of the 128 comparisons, **78 are significant at α=0.05 and 63 survive Bonferroni** (method
significantly *better* than RRF):

| dataset | eval queries | sig. improvements over RRF (of 32) | survive Bonferroni |
|---|:--:|:--:|:--:|
| MSMARCO | ~6,980 | 31 / 32 | 29 / 32 |
| NQ | ~2,893 | 31 / 32 | 29 / 32 |
| NFCorpus | ~323 | 13 / 32 | 5 / 32 |
| ACORD | ~57 | 3 / 32 | 0 / 32 |
...
query-adaptive methods beat not just RRF but a fixed tuned mean-optimal weight (51/128 significant,
