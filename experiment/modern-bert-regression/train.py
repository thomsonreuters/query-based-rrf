...
from transformers import (
    AutoTokenizer, 
    AutoModelForSequenceClassification, # <--- ADD THIS
    Trainer, 
    TrainingArguments,
...
def save_validation_results(trainer, eval_dataset, exp_dir):
    ...
    with torch.no_grad():
        for batch in dataloader:
            ...
            # CHANGE 1: Use keyword arguments (input_ids=..., attention_mask=...)
            outputs = model(
                input_ids=batch['input_ids'], 
                attention_mask=batch['attention_mask']
            )
            
            # CHANGE 2: Access logits directly from the Hugging Face output object
...
def main(config_path, overrides=None):
    ...
    # Replace RobertaTokenizer with AutoTokenizer
    ...
    # Initialize model natively for regression (num_labels=1 automatically triggers MSELoss)
    model = AutoModelForSequenceClassification.from_pretrained(
        config.get('model.name'),
        num_labels=1,
        problem_type="regression",
        classifier_dropout=config.get('model.dropout', 0.1) 
    )
