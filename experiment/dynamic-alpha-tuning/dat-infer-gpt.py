...
# ---------------------------------------------------------------------------
# LLM client abstraction (TR AI Platform token → direct OpenAI endpoint)
# ---------------------------------------------------------------------------

class LLMClient:
    """Thin wrapper that authenticates via TR AI Platform and calls OpenAI directly."""

    @staticmethod
    def from_aiplatform(workspace_id, model_name,
        ...
        client._oai = AsyncOpenAI(api_key=credentials["openai_api_key"], base_url=base_url)
        return client

    async def invoke(self, system_message, user_message):
        response = await self._oai.responses.create(
...
async def main():
    ...
    OUTPUT_DIR = "/home/sagemaker-user/query-aware-rrf/query-based-rrf/data/output/dynamic-alpha-tuning-gpt"

    WORKSPACE_ID = "PracticalLawQW7T"
    MODEL_NAME = "gpt-5.2"

    llm_client = LLMClient.from_aiplatform(
        workspace_id=WORKSPACE_ID,
        model_name=MODEL_NAME,
    )
